#!/usr/bin/env node
// Generates public/consent.js — the standalone cookie-consent + optional
// measurement loader.
//
...
// inside the React app, and it must run before any optional tracking request.
...
import { GA_MEASUREMENT_ID, IMPACT_AFFILIATE_TAG_URL } from "../src/config.js";

const __dirname = dirname(fileURLToPath(import.meta.url));
const OUT = join(__dirname, "..", "public", "consent.js");

const GA_ID = GA_MEASUREMENT_ID;
const CLARITY_ID = String(process.env.VITE_CLARITY_PROJECT_ID || "xr5p94wg3k").trim();
const IMPACT_TAG_URL = IMPACT_AFFILIATE_TAG_URL;
 ...
 *
 * Google Analytics and Impact are NOT loaded until the visitor accepts their
 * respective optional categories. Consent Mode v2 defaults are set to denied
 * before anything else.
 */
(function () {
  var GA_ID = ${JSON.stringify(GA_ID)};
  var CLARITY_ID = ${JSON.stringify(CLARITY_ID)};
  var IMPACT_TAG_URL = ${JSON.stringify(IMPACT_TAG_URL)};
  var STORAGE_KEY = "ac_cookie_consent";
  var MARKETING_STORAGE_KEY = "ac_marketing_consent";
  var RTL_LANGS = ["ar"];
  ...
  function readMarketing() {
    try { return localStorage.getItem(MARKETING_STORAGE_KEY); } catch (e) { return null; }
  }

  function writeMarketing(value) {
    try { localStorage.setItem(MARKETING_STORAGE_KEY, value); } catch (e) { /* private mode */ }
  }
  ...
  // Impact's official tag is loaded only after explicit marketing consent.
  // Keep this exact loader in the global consent asset: it runs once per page
  // load and is shared by the React app and every prerendered static route.
  function loadImpact() {
    if (impactLoaded || !IMPACT_TAG_URL) return;
    impactLoaded = true;
  ...
  function grantClarity() {
    if (!CLARITY_ID) return;
    loadClarity();
    clarityCommand("consentv2", { ad_Storage: "denied", analytics_Storage: "granted" });
  }

  function denyClarity() {
    if (clarityLoaded) {
      clarityCommand("consentv2", { ad_Storage: "denied", analytics_Storage: "denied" });
    }
    try {
  ...
  function show(managing) {
    ...
    if (managing) {
      ...
      marketingLabel.style.cssText = "display:flex;justify-content:space-between;gap:12px;padding:10px;border:1px solid #2A3A55;border-radius:8px;color:#E4EBF5;cursor:pointer";
      var marketingToggle = document.createElement("input");
      marketingToggle.type = "checkbox";
      marketingToggle.checked = readMarketing() === "granted";
      marketingToggle.setAttribute("aria-label", t("marketing"));
      var marketingText = document.createElement("span");
      marketingText.textContent = t("marketing");
      marketingLabel.appendChild(marketingText);
      marketingLabel.appendChild(marketingToggle);
      controls.appendChild(essential);
      controls.appendChild(analyticsLabel);
      controls.appendChild(marketingLabel);
      ...
      save.addEventListener("click", function () {
        ...
        var marketingValue = marketingToggle.checked ? "granted" : "denied";
        var reloadAfterDeny = (value === "denied" && clarityLoaded) || (marketingValue === "denied" && impactLoaded);
        write(value);
        writeMarketing(marketingValue);
        if (value === "granted") grantAnalytics(); else deny();
        if (marketingValue === "granted") grantMarketing();
        close();
        // Clarity cannot unload an injected tag. Reload once after withdrawal so
        // the stored denial takes effect before any analytics tag is initialized.
        if (reloadAfterDeny) location.reload();
      });
  ...
  function start() {
    ...
    if (stored === "granted") {
      grantAnalytics();
      if (readMarketing() === "granted") grantMarketing();
    }
    else if (stored === "denied") deny();
    else show();
...
console.log(`✓ consent asset written: public/consent.js (${Object.keys(STRINGS).join(", ")}; Clarity ${CLARITY_ID ? "enabled" : "awaiting project ID"})`);
