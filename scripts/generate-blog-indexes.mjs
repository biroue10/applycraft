...
function card(article) {
  const meta = `<div class="post-meta"><span class="tag">${esc(article.category)}</span><span><time datetime="${article.datePublished}">${esc(formatEditorialDate(article.datePublished, article.locale))}</time></span><span>· ${article.readMinutes} ${article.locale === "fr" ? "min de lecture" : "min read"}</span></div>`;
