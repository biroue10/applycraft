...
export default function HeroResumeScanner({ copy, rtl = false }) {
  ...
  return <section className="ac-hero-preview ac-resume-scanner" dir={rtl ? "rtl" : "ltr"}>
    <style>{`
      ...
      .ac-scan-back,.ac-scan-paper{position:absolute;width:330px;height:448px;border-radius:16px;background:#fff}
      .ac-scan-back{inset:48px auto auto 50%;transform:translateX(-45%) rotate(6deg);opacity:.38;border:1px solid #a78bfa80;box-shadow:0 28px 80px #02061799}
      .ac-scan-paper{inset:30px auto auto 50%;transform:translateX(-55%);overflow:hidden;color:#182238;border:1px solid #dbe4f0;box-shadow:0 30px 80px #020617b8,0 0 0 6px #ffffff08}
      ...
      .ac-scan-notes{position:absolute;inset:0;z-index:5;pointer-events:none}.ac-scan-note{position:absolute;max-width:160px;padding:9px 12px;border:1px solid #a78bfa66;border-radius:12px;background:#10182bf0;box-shadow:0 14px 34px #02061788;backdrop-filter:blur(8px);color:#eef2ff;font-size:11px;font-weight:750;opacity:0;animation:acNote 6.6s ease infinite}.ac-scan-note-check{margin-inline-end:6px;color:#c4b5fd}.ac-scan-note:nth-child(1){top:26%;inset-inline-end:0}.ac-scan-note:nth-child(2){top:49%;inset-inline-start:0;animation-delay:1.15s}.ac-scan-note:nth-child(3){top:71%;inset-inline-end:2%;animation-delay:2.3s}
      .ac-scan-orbit{position:absolute;inset:80px 22px 30px;border:1px solid #818cf833;border-radius:50%;animation:acOrbit 5s ease-in-out infinite}.ac-scan-orbit-dot{position:absolute;width:7px;height:7px;border-radius:50%;background:#818cf8;box-shadow:0 0 14px #818cf8}.ac-scan-orbit-dot:first-child{top:12%;left:8%}.ac-scan-orbit-dot:last-child{right:5%;bottom:18%;background:#38bdf8}
    ...
    <div role="img" aria-label={copy?.aria}>
      ...
      <div className="ac-scan-notes" aria-hidden="true">{notes.slice(0, 3).map((note) => <div className="ac-scan-note" key={note}><span className="ac-scan-note-check" aria-hidden="true">✓</span>{note}</div>)}</div>
    </div>
  </section>;
}
